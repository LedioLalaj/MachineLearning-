...
def my_backward(x, y_target, w, cache):
    # Cast everything to float64 so gradients match numerical precision
...
def gradient_check():
    ...
    for key in w:
        ...
        for _ in range(5):
            ...
            ana = grads[key][idx]
            w_copy = {k: v.copy() for k, v in w.items()}  # protect w
            num = nn_mod.numerical_gradient(x, y, w_copy, key, idx)
            denom = max(1e-12, abs(num) + abs(ana))
            max_err = max(max_err, abs(num - ana) / denom)
        flag = "OK" if max_err < 1e-2 else "BUG"
